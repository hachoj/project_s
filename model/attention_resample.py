...
class AttentionResample(nn.Module):
    ...
    Note on positional encoding stability:
    The previous implementation used an ``nn.Linear(1, C)`` fed with raw
    flattened pixel indices ``[0..H*W-1]``. With default Linear init
    (fan_in=1), this produced very large positional values (on the order of
    thousands for 64x64 patches), which then got added to features and caused
    severe gradient spikes. To stabilize, we switch to an ``nn.Embedding`` over
    HW positions, which keeps magnitudes bounded and independent of H*W.
    """

    def __init__(self, embd_dim, patch_size=(64, 64), num_heads=8):
        super().__init__()
